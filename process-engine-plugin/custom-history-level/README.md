...
This example demonstrates how to use a custom history level as process engine plugin.
...
method. Additionally in the `postInit` method all register custom history levels are
added to the `PerProcessHistoryLevel` instance. This way also other custom history levels
are registered.
...
The idea for the `custom-variable` history level is that only few variables are
...
history all others are ignored. For the rest events (Process Instance, Activity
Instance) a full history is written.
...
In the [CustomVariableHistoryLevelTest][4] it is demonstrated that only the
correct variables are saved to history. In the
[PerProcessVariableHistoryLevelTest][5] several process are deployed with
