...
public class TestSynchronousServiceTask {
	...
	public void testServiceInvocationSuccessful() {
		...
		Map<String, Object> variables = Collections.<String,Object>singletonMap(
				SynchronousServiceTask.SHOULD_FAIL_VAR_NAME, false
		);
		...
		// This method invocation returns after the service task has been executed and the process
		// instance has advanced to the second waitstate.
	...
	public void testServiceInvocationFailure() {
		...
		Map<String, Object> variables = Collections.<String,Object>singletonMap(
		  SynchronousServiceTask.SHOULD_FAIL_VAR_NAME, true
		);
		...
		// Complete this task. This triggers the synchronous invocation of the service task.
		// This time the service task will fail and the process instance will roll back to it's previous state:
		try {
			taskService.complete(waitStateBefore.getId());
		...
		// the process instance is still waiting in the first wait state (user task):
