...
public class BankCustomerProcessTest {
	...
	public void process_inputJaxbObjectInProcess_javaObjectPassedToService() throws Exception {
		...
		String singleSignonToken = "abc";
		
		GetAccountsRequest inputRequest = new GetAccountsRequest();
		inputRequest.setCustomerNumber(customerNumber);
		
		// invoke process (async behaviour)
		Map<String, Object> variables = new HashMap<>();
		variables.put("request",  inputRequest);
		variables.put("singleSignonToken",  singleSignonToken);		
		...
		verifyRequest(customerNumber, singleSignonToken);
		...
		GetAccountsResponse response = (GetAccountsResponse)historyService.createHistoricVariableInstanceQuery().processInstanceId(processInstance.getProcessInstanceId()).variableName("response").singleResult().getValue();;
	...
	private void verifySerializationFormat(ProcessInstance processInstance, String format) {
		...
		for(HistoricVariableInstance item : list) {
		  if(item.getName().equals("request") || item.getName().equals("response")) {
	...
	private void verifyRequest(String customerNumber, String singleSignonToken) throws Exception {
		...
		assertThat(header.getSingleSignonToken(), is(singleSignonToken));
	}	
